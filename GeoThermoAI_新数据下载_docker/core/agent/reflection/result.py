...
class ReflectionResult:
    ...
    def with_rule(self, rule_id: str, violation: str = "") -> "ReflectionResult":
        """返回追加了规则命中记录的新结果（不可变）。"""
        return ReflectionResult(
            ok=self.ok, action=self.action, question=self.question, note=self.note,
            violations=self.violations + ([violation] if violation else []),
            suggestions=list(self.suggestions),
            rule_hits=self.rule_hits + [rule_id],
            data=dict(self.data),
        )
