...
class TableWriter:
    ...
    def write_record_batch(self, batch: pa.RecordBatch):
        """写入/追加一个 RecordBatch（schema 与输入文件一致时最稳）。"""
        table = pa.Table.from_batches([batch])
        writer = self._ensure_open(table)
        table = table.cast(writer.schema)
        writer.write_table(table)
        self.wrote_rows += batch.num_rows
...
def write_table(path, df, append=False, compression="zstd"):
    """写入（或追加）一个 DataFrame 到 Parquet。"""
    table = pa.Table.from_pandas(df, preserve_index=False)
    if append and os.path.exists(path) and os.path.getsize(path) > 0:
        pf = pq.ParquetFile(path)
        with pq.ParquetWriter(path, pf.schema_arrow, compression=compression) as writer:
            table = table.cast(writer.schema)
            writer.write_table(table)
    else:
        table.to_parquet(path, index=False, compression=compression)
