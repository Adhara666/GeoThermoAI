...
B-10 / 用户确认第9条：本轮只接入 Landsat ST_QA（温度不确定度）写入核心 CSV/meta 并
报告分布统计，不做任何硬筛选、不自创权重阈值，也不改变 joint_mask。
"""
...
ST_QA_SCALE = 0.01              # Landsat Collection 2 ST_QA 缩放因子（USGS 文档：0.01 K/DN）
EPS = 1e-8                      # 防止除零
...
def train_columns(has_st_qa: bool) -> List[str]:
    cols = list(_BASE_TRAIN_COLUMNS)
    if has_st_qa:
        cols.append(ST_QA_COLUMN)
    cols.append(TARGET_COLUMN)
    return cols


def constraint_columns(has_st_qa: bool) -> List[str]:
    cols = list(_BASE_CONSTRAINT_COLUMNS)
    if has_st_qa:
        cols.append(ST_QA_COLUMN)
    return cols
...
def process_preprocessing(
    ...
    Args:
        ...
        output_dir:     输出目录
        st_qa_path:     Landsat ST_QA（温度不确定度）栅格路径，可选（B-10/确认第9条：
                        本轮只接入并报告分布统计，不提供时该功能整体跳过，不影响主流程）
    ...
    if st_qa_path:
        st_qa_path = _resolve_raster_path(st_qa_path, progress_callback)
    ...
    try:
        ...
        has_st_qa = bool(st_qa_path)
        if has_st_qa:
            with rasterio.open(st_qa_path) as _chk:
                if (_chk.height, _chk.width) != (height_30m, width_30m):
                    raise ValueError(
                        f"ST_QA 栅格尺寸 {(_chk.height, _chk.width)} 与 Landsat 参考格网 "
                        f"{(height_30m, width_30m)} 不一致，本轮跳过 ST_QA 接入"
                    )
        ...
        st_qa_k = np.full((height_30m, width_30m), np.nan, dtype=np.float32) if has_st_qa else None
        ...
        with rasterio.open(landsat_path) as lst_ds, \
            ...
            st_qa_ds = rasterio.open(st_qa_path) if has_st_qa else None
            st_qa_nodata = st_qa_ds.nodata if st_qa_ds is not None else None
            try:
                for start in range(0, height_30m, block_rows_30m):
                    ...
                    # ST_QA（B-10：仅接入+统计，不参与掩膜/训练）
                    if st_qa_ds is not None:
                        qa_block = st_qa_ds.read(1, window=window).astype(np.float32)
                        if st_qa_nodata is not None:
                            qa_block[qa_block == st_qa_nodata] = np.nan
                        st_qa_k[start:stop, :] = qa_block * ST_QA_SCALE
            finally:
                if st_qa_ds is not None:
                    st_qa_ds.close()
        ...
        if has_st_qa:
            train_data[ST_QA_COLUMN] = st_qa_k[::step, ::step][mask_ss]
        train_data[TARGET_COLUMN] = lst_k

        df_train = pd.DataFrame(train_data)[train_columns(has_st_qa)]
        df_train = df_train.replace([np.inf, -np.inf], np.nan).dropna(
            subset=[c for c in df_train.columns if c != ST_QA_COLUMN]
        )
        df_train.to_csv(train_csv, index=False)

        # 生成30m训练集元数据（含 ST_QA 分布统计，B-10）
        st_qa_stats = None
        if has_st_qa and ST_QA_COLUMN in df_train.columns:
            qa_series = df_train[ST_QA_COLUMN].dropna()
            if len(qa_series) > 0:
                st_qa_stats = {
                    "unit": "K",
                    "count": int(len(qa_series)),
                    "min": round(float(qa_series.min()), 4),
                    "median": round(float(qa_series.median()), 4),
                    "p90": round(float(qa_series.quantile(0.90)), 4),
                    "p95": round(float(qa_series.quantile(0.95)), 4),
                    "max": round(float(qa_series.max()), 4),
                    "note": "仅接入并报告分布统计，未用于筛选或加权训练样本（B-10 本轮范围）",
                }
        ...
        with rasterio.open(landsat_path) as ds:
            train_meta_dict = {
                ...
                "ravel_order": "C",
                "step": step,
                "columns": train_columns(has_st_qa),
                "has_st_qa": has_st_qa,
                "st_qa_distribution": st_qa_stats,
                "sentinel2_sr_scale": (
        ...
        if has_st_qa:
            constraint_data[ST_QA_COLUMN] = st_qa_k[joint_mask]

        df_constraint = pd.DataFrame(constraint_data)[constraint_columns(has_st_qa)]
        df_constraint = df_constraint.replace([np.inf, -np.inf], np.nan).dropna(
            subset=[c for c in df_constraint.columns if c != ST_QA_COLUMN]
        )
        ...
        def _validate_constraint(tmp_path: str) -> Tuple[bool, str]:
            ...
            missing = [c for c in constraint_columns(has_st_qa) if c not in check.columns]
            if missing:
        ...
        constraint_meta_dict = {
            ...
            "ravel_order": "C",
            "columns": constraint_columns(has_st_qa),
            "has_st_qa": has_st_qa,
