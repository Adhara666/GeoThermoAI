...
class DataPipelineSkill(BaseSkill):
    ...
    def parameters(self) -> List[SkillParameter]:
        return [
            ...
            SkillParameter(name="st_qa_path", type="file_path", description="Landsat ST_QA 栅格路径（可选，B-10）", required=False),
    ...
    def execute(
        ...
        st_qa_path = params.get("st_qa_path") or None
        if not st_qa_path and landsat_path:
            # Agent 的 SKILL_PATHS（未修改）不会主动注入 st_qa_path；按固定命名约定
            # 从 landsat_path 所在目录（raw_dir）探测 data_acquisition 是否已下载
            # landsat_st_qa.tif（B-10 可选波段，不存在时静默跳过，不影响主流程）。
            _raw_dir = os.path.dirname(landsat_path)
            _candidate = os.path.join(_raw_dir, "landsat_st_qa.tif")
            if os.path.isfile(_candidate):
                st_qa_path = _candidate
        ...
        # ── 步骤1: 数据预处理（含完整30m约束层，A-05；可选 ST_QA，B-10）───────
        if log_callback:
        ...
        result_data = {
            ...
            "has_st_qa": prep_result.get("has_st_qa", False),
        }
        ...
        if project_root:
            run_manifest.record_stage(
                ...
                stats={
                    ...
                    "has_st_qa": result_data["has_st_qa"],
