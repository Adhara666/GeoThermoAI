    ...
    <div v-if="hasClosure" class="acc-section">
      ...
      <template v-else>
        <table class="metric-table">
          <tbody>
            <tr><td>平均偏差 (K)</td><td>{{ fmtSigned(closure.data.closure.metrics.MB_K) }}</td></tr>
            <tr><td>MAE (K)</td><td>{{ fmt(closure.data.closure.metrics.MAE_K) }}</td></tr>
            <tr><td>RMSE (K)</td><td>{{ fmt(closure.data.closure.metrics.RMSE_K) }}</td></tr>
            <tr><td>对照格网数</td><td>{{ closure.data.closure.n_matched_cells?.toLocaleString?.() ?? closure.data.closure.n_matched_cells }}</td></tr>
            <tr><td>覆盖率</td><td>{{ fmt(closure.data.closure.coverage_ratio, 2) }}</td></tr>
          </tbody>
        </table>
