...
# ---- synthetic ST_QA (optional B-10 input): small uncertainty around 0.3K ----
st_qa_dn = (rng.uniform(20, 80, size=dem.shape)).astype(np.uint16)  # *0.01K -> 0.2~0.8K
write_raster(os.path.join(RAW_DIR, "landsat_st_qa.tif"), st_qa_dn, transform_30m, "uint16")
...
result = skill.execute({
    ...
    "st_qa_path": os.path.join(RAW_DIR, "landsat_st_qa.tif"),
...
assert result.data["has_st_qa"] is True
...
assert "ST_QA" in train_df.columns, "ST_QA column should be present in step2 CSV when st_qa_path provided"
...
final_df = pd.read_csv(os.path.join(RESULTS_DIR, "rf_10m_predict.csv"))
