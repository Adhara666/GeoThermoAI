...
def fetch_scene_calibration(signed_item, log_callback=None, timeout: int = 30) -> Dict:
    ...
    asset = signed_item.assets.get("product-metadata")

    if asset is not None and getattr(asset, "href", None):
        try:
            resp = requests.get(asset.href, timeout=timeout)
