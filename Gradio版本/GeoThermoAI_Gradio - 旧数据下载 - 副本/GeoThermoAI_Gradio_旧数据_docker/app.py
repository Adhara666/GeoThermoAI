...
def build_ui(api: GradioAPI):
    ...
    with gr.Blocks(title="GeoThermoAI", css=CSS) as demo:
        ...
        with gr.Row(elem_classes="main-row"):
            ...
            with gr.Column(scale=30, min_width=310):
                ...
                with gr.Tabs():
                    ...
                    with gr.Tab("⬇️ 下载"):
                        dl_dir_label = gr.Markdown("📂 当前项目目录：未设置")
                        dl_refresh_btn = gr.Button("🔄 刷新文件列表", size="sm")
                        dl_file_choices = gr.Dropdown(
                            label="选择要下载的文件（含子目录）",
                            choices=[],
                            interactive=True,
                        )
                        dl_download_btn = gr.Button("⬇️ 下载选中文件", variant="primary", size="sm")
                        dl_link = gr.HTML(
                            "<span style='font-size:0.9em;color:#888;'>下载链接将在选择文件并点击下载后生成</span>"
                        )
                        dl_status = gr.Markdown(
                            "使用步骤：\n"
                            "1) 左侧填好「项目保存路径」并点保存\n"
                            "2) 点「刷新文件列表」，下拉框会列出项目目录内所有文件（子目录文件带「子目录/文件名」前缀）\n"
                            "3) 选中文件后点「下载选中文件」，再点生成的下载链接（若被拦截，复制链接地址到新标签页打开）"
                        )
        ...
        # 文件下载：刷新列表 / 下载选中文件 / 切换项目时自动同步列表
        dl_refresh_btn.click(
            api.list_project_files, [proj_dir], [dl_file_choices, dl_dir_label, dl_status],
        )
        dl_download_btn.click(
            api.prepare_download, [proj_dir, dl_file_choices], [dl_link, dl_status],
        )
        proj_dir.change(
            api.list_project_files, [proj_dir], [dl_file_choices, dl_dir_label, dl_status],
        )
...
def main():
    api = GradioAPI()
    demo = build_ui(api)
    # 下载功能：把下载临时目录登记为 Gradio 允许访问的路径，
    # 否则文件接口 /gradio_api/file= 会拒绝访问该目录下的文件
    import tempfile
