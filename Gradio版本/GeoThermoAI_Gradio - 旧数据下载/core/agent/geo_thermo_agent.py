...
class GeoThermoAgent:
    ...
    def process_command(self, user_input: str, on_token=None, pause_callback=None, project_dir: str = "", workflow_callback=None) -> str:
        """处理用户自然语言指令
        ...
        # 4. 调用 LLM 生成执行计划（低温度+短max_tokens，加速结构化输出）
        ...
        return self._execute_plan(plan, on_token=on_token, pause_callback=pause_callback, project_dir=project_dir, workflow_callback=workflow_callback)
    ...
    def _normalize_plan_paths(self, plan: dict) -> None:
        ...
        # 避免 LLM 生成错误的相对路径导致解析失败。
        study_area_file = self._find_study_area_file()
        if study_area_file:
            for step in steps:
                if step.get("skill") == "data_acquisition":
                    region = step.get("params", {}).get("region", "")
                    if not region or self._looks_like_file_path(region):
                        step["params"]["region"] = study_area_file
                    break

    # ── 执行引擎 ─────────────────────────────────────────────────────

    def _execute_plan(self, plan: dict, on_token=None, pause_callback=None, project_dir: str = "", workflow_callback=None) -> str:
        ...
        _emit_accumulator = []  # list as mutable container for accumulated output
        ...
        for i, step in enumerate(steps):
            ...
            _emit(f"**Step {i+1}/{total}**: {skill_name}\n")
            # 更新工作流进度（running）
            ...
            try:
                ...
                if skill_name == "data_acquisition" and not step.get("params", {}).get("selected_pair"):
                    ...
                    if pairs:
                        ...
                        _emit(f"  ⚠️ 未找到影像配对，后续步骤终止\n")
                        results.append(f"{skill_name}: 未找到符合条件的影像配对")
                        return "\n".join(results)
                else:
                    # 普通执行或其他 Skill
                    result = skill.execute(
                ...
                if skill_name == "data_acquisition" and result.success:
                    acquisition_outputs = dict(result.data)
                    # 确保 output_dir 存在
                ...
                if skill_name == "data_pipeline" and result.success:
                    data_features = self._collect_data_features(result.data)
...
## 路径格式（重要）
所有文件路径必须使用相对路径，以 `./` 开头。
正确示例：`./output/wuhan_lst_202407/raw/landsat_lst.tif`
错误示例：`/output/wuhan_lst_202407/raw/landsat_lst.tif`
