...
class DataAcquisitionSkill(BaseSkill):
    ...
    def execute(
        ...
        if ds_cfg.get("client_id") and ds_cfg.get("client_secret"):
            try:
                ...
                ds_token = ""
                if log_callback:
                    log_callback("WARN", f"Data Space 认证失败: {e}，Sentinel-2 回退 Planetary Computer")
        ...
        # Data Space 下载需要 Bearer token；未配置/失败则 ds_token 为空 → 走 Planetary Computer SAS
    ...
    @staticmethod
    def _get_dataspace_token(cfg: dict) -> str:
        """获取 Copernicus Data Space OAuth2 access_token（client_credentials 模式）"""
        resp = requests.post(
            _DS_TOKEN_URL,
            data={
    ...
    def _download_composite(
        ...
        def _download_one(idx, item, b):
            ...
            if auth_headers:
                # Data Space：asset 无需 SAS 签名，直接带 Bearer token 下载
                ...
                if not asset:
                    return idx, b, f"{item.id}: 缺少波段 {b}", None
                href = asset.href
            data_bytes = self._fetch_asset(href, log_callback, headers=auth_headers)
