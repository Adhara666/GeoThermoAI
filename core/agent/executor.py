...
def execute_plan(agent, plan: dict, on_token=None, on_log=None, pause_callback=None,
    ...
    while i < total:
        ...
        if skill_name in MODEL_TRAIN_SKILLS:
            ...
            # LLM 生成的 params 可能是 null，先归一为 dict 再写入
        ...
        while True:
                ...
                with open("/proc/self/status") as _f:
                    for _line in _f:
                        if _line.startswith("VmRSS:"):
                            logger.info("[memtrim] 步骤 %s 执行前 RSS=%s KB", skill_name, _line.split()[1])
                            break
            except Exception:
                pass
            ...
            try:
                        ...
                        if pairs:
                            ...
                            if hooks is not None:
                                _detail = {
                                    ...
                                    "cloud_stats": result_data.get("cloud_stats"),
                                    "composite": step.get("params", {}).get("composite", "pair"),
                                }
                ...
                # 本步完成后立即把空闲堆归还 OS：RF 训练/TTRI/TCR 等大内存步骤
                # 一结束就回落峰值（而不是等到下一步执行前/整轮结束才释放），
                # 尤其覆盖 lst_export（生成 10m 有空洞 LST 后）与 lst_gapfill
                # （结果后处理完成后）两个用户可见的「产品已生成」节点，避免
                # 流程虽完成但进程 RSS 仍占着几个 G 不回落。
                # 先 gc.collect() 断循环引用，再 malloc_trim 归还空闲堆页。
                gc.collect()
                release_rss_memory()
                try:
                    with open("/proc/self/status") as _f:
                        for _line in _f:
                            if _line.startswith("VmRSS:"):
                                logger.info("[memtrim] 步骤 %s 执行后 RSS=%s KB", skill_name, _line.split()[1])
                                break
                except Exception:
                    pass
                ...
                if decision is None or decision.action == StepDecision.CONTINUE:
                    if hooks is None:
                        ...
                        if not should_continue:
                            ...
                            _emit(presentation.waiting_for_user())
                            _finalize_experiment("paused")
                            _cleanup_workflow_memory(ctx, exp_state)
                            return "\n".join(results) + f"\n{PAUSE_MARKER}"
                    break
    ...
    if exp_state["paused"]:
        ...
        if failed is not None:
            ...
            # partial 意图：全部步骤成功后提示用户可以继续后续流程
            if plan.get("intent") == "partial":
                _msg = _partial_continuation_hint(plan)
                if _msg:
                    _emit(presentation.step_gap())
                    _emit(_msg)

    # 实验记录已落盘，exp_state 里的大对象不再需要，统一释放
    _cleanup_workflow_memory(ctx, exp_state)

    return "\n".join(results)


def _cleanup_workflow_memory(ctx: ExecContext, exp_state: dict) -> None:
    """工作流退出时统一释放大对象引用 + 刷缓存 + 归还空闲堆。

    在正常完成、暂停（PAUSE）、中止（ABORT）、重规划（REPLAN）四条退出路径
    都必须调用，否则 exp_state 里的大对象（rf_data / acc_data / data_features）
    会被长期持有，gc 无法回收，glibc arena 里的空闲页也无法被 malloc_trim 归还
    OS（实测暂停后 RSS 停在 3.8GB 不回落，手动 trim 才降到 1.9GB）。
    """
    logger.info("[memtrim] 退出路径内存清理开始")
    for _k in ("rf_data", "acc_data", "data_features", "pair", "acq_params",
               "tuning_trace", "pair_candidates", "final_params"):
        exp_state.pop(_k, None)
    ctx.data_features = None
    ctx.last_pairs = []
    ctx.replan_payload = {}
    ctx.notes = []

    # 清理瓦片渲染 lru_cache（可视化面板浏览结果后 4096 张 PNG 常驻）
    try:
    ...
    # 刷 GDAL 全局块缓存（各 Skill 只 FlushCache 单个数据集，全局缓存仍持有
    # 已关闭 GeoTIFF 的解码块）
    try:
    ...
    try:
        with open("/proc/self/status") as _f:
            for _line in _f:
                if _line.startswith("VmRSS:"):
                    logger.info("[memtrim] 退出路径清理后 RSS=%s KB", _line.split()[1])
                    break
    except Exception:
        pass


# partial 完成后建议的后续步骤（按 WORKFLOW_STEPS 顺序）
_NEXT_STEP_HINT = {
    "data_acquisition": "数据预处理",
    "data_pipeline": "模型训练",
    "ttri_compute": "模型训练",
    "rf_model": "热约束残差计算与产品导出",
    "tcr_compute": "地表温度产品导出",
    "lst_export": "精度评估",
    "accuracy_eval": "结果后处理（空洞填补）",
}


def _partial_continuation_hint(plan: Dict[str, Any]) -> str:
    """partial 意图全部步骤成功后，提示用户可以继续后续流程。

    根据已执行的最后一个步骤，建议下一个合理的步骤。
    用引用块样式（与结果后处理提示一致），不弹审批卡片——用户可以自然语言继续。
    """
    ...
    if not steps:
        return ""
    last_skill = str((steps[-1] or {}).get("skill") or "")
    # 找到最后一个已执行步骤在 WORKFLOW_STEPS 中的位置，取下一个未执行的
    all_steps = plan_schema.WORKFLOW_STEPS
    executed = {str((s or {}).get("skill") or "") for s in steps}
    next_step = ""
    for name in all_steps:
        if name not in executed:
            ...
            break
    if not next_step:
        # 全部7步都执行了（不太可能，但兜底）：建议结果后处理
        return "> 本次部分流程已全部完成。如需对结果做空洞填补，告诉我即可\n"

    next_label = _NEXT_STEP_HINT.get(last_skill, "")
    if not next_label:
        next_label = presentation.stage_label(next_step)
    return (f"> 本次部分流程已完成。如需继续，可以说「{next_label}」"
            f"或「继续后续流程」，我会接着往下跑\n")
...
def _handle_control_decision(decision: StepDecision, ctx: ExecContext, results: List[str],
    ...
    所有提前返回路径都先调 _cleanup_workflow_memory 释放大对象，避免暂停/
    中止后 RSS 停在峰值不回落。
    """
    if decision.action == StepDecision.PAUSE:
        ...
        finalize("paused")
        _cleanup_workflow_memory(ctx, ctx.exp_state)
        return "\n".join(results) + f"\n{PAUSE_MARKER}"
