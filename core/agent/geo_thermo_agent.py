...
from . import executor, presentation
...
class GeoThermoAgent:
    ...
    def process_command(self, user_input: str, on_token=None, on_log=None, pause_callback=None, project_dir: str = "", workflow_callback=None, settings_path: str = "", study_areas_dir: str = "", conv_id: str = "", project_id: str = "", memory_manager=None, exec_mode: str = "", prior_messages=None, session_state=None, on_thinking=None) -> str:
        ...
        # 咨询判定统一走 keywords.is_advisory_request（I1）
        _advisory = _is_advisory_request(user_input)
        if _advisory:
        ...
        #    结构化输出：优先使用 response_format 保证返回合法 JSON（json_mode=True）
        response = self.assistant._call_api([
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": user_input},
        ], temperature=0.1, max_tokens=4096, on_thinking=on_thinking, json_mode=True)
        ...
        if plan is None:
            ...
            response = self.assistant._call_api([
                ...
                {"role": "user", "content": user_input},
            ], temperature=0.0, max_tokens=4096, on_thinking=on_thinking, json_mode=True)
            plan = self._parse_plan(response)
        if plan is None:
    ...
    def _guess_region_from_input(self, user_input: str) -> dict:
        ...
        # 城市→bbox 统一来源（I3）：从 keywords.py 导入，可配置扩展
        bbox = guess_city_bbox(user_input)
    ...
    def _normalize_plan_paths(self, plan: dict, study_areas_dir: str = "") -> None:
        ...
        # 修正每个步骤的所有路径
        for step in steps:
            # LLM 生成的 params 可能是 null，先归一为 dict（旧路径不经过 plan_schema 规范化）
