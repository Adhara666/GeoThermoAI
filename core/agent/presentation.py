...
# server 侧工作流面板用的短标签（沿用既有面板文案，不改动显示）
...
def no_pair_reason(detail: Dict[str, Any]) -> str:
    ...
    优化3：区分"完全搜到0景"和"搜到了但配对不合格"两种情况。
    """
    detail = detail or {}
    lc = detail.get("landsat_count")
    sc = detail.get("sentinel_count")
    # 优化3：0景时给出不同的提示
    if (not lc or lc == 0) and (not sc or sc == 0):
        return ("该时间段内未搜索到任何影像（Landsat 0 景、Sentinel-2 0 景）。"
                "请确认时间范围是否正确，或尝试扩大搜索范围。\n")
    lines = [
        f"没有找到符合条件的影像组合：Landsat 8/9 {fmt_count(lc)} 景，"
        f"Sentinel-2 {fmt_count(sc)} 景。"
    ]
    reasons: List[str] = []
