...
`MAX_TUNING_ROUNDS`（硬上限 8）与 `DEFAULT_TUNING_ROUNDS`（默认 5）定义在
`orchestrator/agent_config.py`，本模块从那里导入。
"""

from typing import Any, Dict, List, Optional, Tuple

from ..orchestrator.agent_config import DEFAULT_TUNING_ROUNDS, MAX_TUNING_ROUNDS, AGENT_DEFAULTS
...
# R1 / R2 阈值（I2：从 agent_config 默认值读取，保持现有行为不变）
R2_FORCE_TUNE_BELOW = AGENT_DEFAULTS["r2_force_tune_below"]
R2_FORBID_TUNE_ABOVE = AGENT_DEFAULTS["r2_forbid_tune_above"]
# R3 过拟合判据
OVERFIT_GAP = AGENT_DEFAULTS["overfit_gap"]
# R5 收敛判据
CONVERGE_DELTA = AGENT_DEFAULTS["converge_delta"]
...
def advisory_notes(data_features: Optional[dict],
    ...
    # I2：阈值从 agent_config 默认值读取
    _large = AGENT_DEFAULTS["large_sample_threshold"]
    _small = AGENT_DEFAULTS["small_sample_threshold"]
    _complex = AGENT_DEFAULTS["complex_terrain_threshold"]
    _flat = AGENT_DEFAULTS["flat_terrain_threshold"]
    _lst_std = AGENT_DEFAULTS["high_lst_std_threshold"]
    _ndvi = AGENT_DEFAULTS["high_ndvi_threshold"]
    _ttri = AGENT_DEFAULTS["low_ttri_threshold"]
    ...
    if isinstance(ttri, (int, float)) and ttri < _ttri:
