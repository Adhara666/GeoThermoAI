...
from .project_state import derive_project_dir, scan_project_state
...
class DataAgent(RoleAgent):
    ...
    def reflect(self, *, raw_dir: str = "", processed_dir: str = "",
        ...
        explained = self._explain(
            result, project_state=scan_project_state(
                derive_project_dir(raw_dir, processed_dir)))
    ...
    def _explain(self, result: ReflectionResult, project_state: str = "") -> Dict[str, Any]:
        findings = "\n".join(f"- 不合格：{v}" for v in result.violations) or "- 无"
        candidates = "\n".join(f"- {s}" for s in result.suggestions) or "- 无"
        parsed = self.call_json(
            data_prompts.reflect_prompt(
                findings, candidates, project_state,
                history_hint=self.memory_block("数据 云量 阈值 历史 配对 建议")),
