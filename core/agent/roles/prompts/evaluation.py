...
## 当前项目已有状态（仅供判断历史对比，不得引用其中的任何数字/日期/数量/路径）
{project_state}

## 系统评级（用于把握语气，不得在输出中写出评级词）
本次测试集精度评级为：{expected_grade}。
评级含义参考：{grade_guidance}

## 你要写三个定性小节（每节 1~2 句，全部不得出现任何数字）：
1. quality_note：数据质量提示（如有效像元覆盖、云空洞情况、整体精度表现）
2. features_note：关键特征与局限（贡献最大的特征、分辨率尺度效应、云掩膜局限等）
3. usage_note：应用建议（结合系统评级把握语气，如适合/谨慎用于何种场景）

## 输出格式（只输出这个 JSON 对象）
{{"quality_note": "...", "features_note": "...", "usage_note": "..."}}

## 写作要求
- 每句都是定性概括；全文不得出现任何指标数值（决定系数、误差、像元数、温度值、
  占比等一律不写）；需要举例时不写具体数值；分辨率单位一律写 10m/30m
- 不得写出评级词（「优秀/良好/一般/偏低」等）；用「精度较高/中等/较低」这类
  定性描述把握语气
...
def qualitative_prompt(knowledge: str, facts: str, expected_grade: str,
                       project_state: str = "", grade_guidance: str = "") -> str:
