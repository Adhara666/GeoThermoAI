...
def _cloud_stats(landsat_items, sentinel2_items) -> dict:
    """搜索结果的云量分布摘要（供 replan 的 LLM 诊断失败原因）。

    输入是 STAC Item 列表；输出形如：
    {"landsat": [45.0, 62.0, ...], "sentinel": [30.5, ...]}
    ——按云量升序排列的纯数值列表，缺失的景跳过。
    只取原始云量，不做任何四舍五入之外的加工；空列表表示该源 0 景。
    """
    def _clouds(items):
        values = []
        for it in (items or []):
            try:
                v = float((it.properties or {}).get("eo:cloud_cover"))
            except (TypeError, ValueError):
                continue
            if 0 <= v <= 100:
    ...
    return {"landsat": _clouds(landsat_items), "sentinel": _clouds(sentinel2_items)}
...
class DataAcquisitionSkill(BaseSkill):
    ...
    def _execute_impl(
        ...
        if not selected_pair:
            ...
            # 优化1：月度合成搜索到 0 景时提前告知，不进入下载阶段白跑
            ...
            if is_monthly:
                ...
                # 优化3：区分"0景"和"有景但配对不合格"
                if not landsat_items and not sentinel2_items:
                    message = (f"该时间段内未搜索到任何影像（Landsat 0 景、Sentinel-2 0 景）。"
                               f"请确认时间范围是否正确，或尝试扩大搜索范围。"
                               f"（耗时 {elapsed:.1f}s）")
                else:
                    message = (f"找到 {len(landsat_items)} 景 Landsat、{len(sentinel2_items)} 景 Sentinel-2，"
        ...
        if os.path.isfile(region):
            ...
            if ext == ".shp":
                try:
                    ...
                    ds = ogr.Open(region)
                    if ds is None:
                        raise OSError("cannot open shapefile")
                    lyr = ds.GetLayer(0)
                    # 读取 .prj 坐标参考；有 CRS 时统一重投影到 WGS84 再取外接矩形，
                    # 避免投影坐标系（米制）坐标被当成经纬度度数
    ...
    def _build_pairs(landsat_items: list, sentinel2_items: list,
        ...
        def _satellite_type(item):
            ...
            if item_id.startswith("lc08") or "landsat_8" in item_id:
                ...
                return "S2B"
            # 属性兜底：部分 STAC 源的 item.id 格式不含卫星代号，
            # 但 properties.platform / constellation 可用
            platform = str((item.properties or {}).get("platform") or "").lower()
            if "sentinel-2a" in platform or platform == "s2a":
                return "S2A"
            if "sentinel-2b" in platform or platform == "s2b":
                return "S2B"
            constellation = str((item.properties or {}).get("constellation") or "").lower()
            if "sentinel" in constellation:
                return "S2"
            return "?"

        def _scene_info(item, default_sat="?"):
            dt = item.properties.get("datetime", "?")[:10]
            cloud = item.properties.get("eo:cloud_cover", None)
            sat = _satellite_type(item)
            if sat == "?":
                sat = default_sat
            return {
                "id": item.id,
        ...
        for lg in landsat_groups:
            ...
            for sg in sentinel_groups:
                s_date = sg["date"]
                s_items = sg["items"]
                s_scenes = [_scene_info(i, default_sat="S2") for i in s_items]
    ...
    def _warp_cloud_to_grid(self, url: str, out_path: str, *,
        ...
        utm = utm_epsg or utm_epsg_for_region(bbox, study_area_geojson)
    ...
    def _download_composite(
        ...
        # ── 1. 计算 UTM zone（研究区 GeoJSON 质心优先，bbox 中心兜底）───
        utm_epsg = utm_epsg_for_region(bbox, study_area_geojson)
