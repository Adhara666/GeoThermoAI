...
def _fmt_num(value, digits=2) -> str:
    """安全格式化数值统计：None/缺失返回 'N/A'，避免对缺失字段做 :.2f 崩溃。"""
...
def _fmt_pct(value) -> str:
    """安全格式化百分比统计（0~1 → 百分数）；None 返回 'N/A'。"""
    if isinstance(value, (int, float)) and not isinstance(value, bool):
...
class LSTGapFillSkill(BaseSkill):
    ...
    def execute(
        ...
        if log_callback:
            log_callback(
                "INFO",
                f"空洞填补完成: 空洞 {_fmt_num(stats.get('filled_pixels'), 0)} 像元"
                f"（{_fmt_pct(stats.get('filled_ratio'))}），"
        ...
        _after = stats.get("after") if isinstance(stats.get("after"), dict) else {}
        return SkillResult(
            success=True,
            message=(
                f"结果后处理完成：填补空洞 {_fmt_num(stats.get('filled_pixels'), 0)} 个像元"
                f"（占总像元 {_fmt_pct(stats.get('filled_ratio'))}），"
                f"填洞后温度范围 {_fmt_num(_after.get('min'))}–"
                f"{_fmt_num(_after.get('max'))} K；"
