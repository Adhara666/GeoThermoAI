...
// 拖拽期间注册的全局监听器清理句柄（组件卸载时强制移除，防泄漏）
...
const tabs = computed(() => [
  { id: 'api', label: t('wb.api'), icon: '<path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/>' },
  { id: 'datasource', label: t('wb.datasource'), icon: '<ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/>' },
...
const modelTag = computed(() =>
  settings.configured ? `🟢 ${settings.displayName}` : `⚪ ${t('wb.noModel')}`,
)
</script>
...
<template>
  ...
  >
    ...
    <div class="workbench__footer">
      <button class="workbench__collapse" :title="t('wb.collapseTitle')" @click="emit('close')">
      ...
      <div class="workbench__meta">
        <div>{{ t('wb.currentModel', { tag: modelTag }) }}</div>
        <div>{{ t('wb.footer') }}</div>
      </div>
    </div>
  </aside>
