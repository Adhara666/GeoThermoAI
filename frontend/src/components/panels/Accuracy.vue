...
async function refresh() {
  ...
  try {
    ...
    if (test.value.status === 'missing' && !test.value.data) test.value = { status: 'error', message: '读取失败，请稍后重试' }
    if (closure.value.status === 'missing' && !closure.value.data) closure.value = { status: 'error', message: '读取失败，请稍后重试' }
  } finally {
    loading.value = false
  }
}
...
<template>
  <div>
    ...
    <p v-if="!hasAny" class="form-hint" style="margin-top:10px">
      {{ loading ? '精度数据加载中…' : '暂无精度数据，请先运行完整流程' }}
    </p>
