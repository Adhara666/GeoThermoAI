...
async function doDownload() {
  ...
  progress.value = 0
  try {
    // 与单文件下载同口径：token 用 getToken()（含内存兜底，隐私模式可用），
    // 且同时带 ?token= query——ModelScope 反代会剥离 Authorization header
    const token = getToken()
    const url = '/api/download/multiple' + (token ? `?token=${encodeURIComponent(token)}` : '')
