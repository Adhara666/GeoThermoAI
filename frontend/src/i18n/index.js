...
const zh = {
  ...
  'chat.thinkingEmpty': '本阶段用于理解、编排、位置解析与结果统计等处理，共用时 {sec} 秒（未产生可展开的模型思考文本）。',
  'chatInput.collapsePath': '点击收起路径',
  'chatInput.fullPath': '点击查看完整路径',
  'chatInput.streamingPh': '回复生成中…',
  'chatInput.pointChip': '已选点（{lon}, {lat}）',
  'chatInput.pointClear': '清除',
  'chatInput.pointTitle': '地图选点：可直接提问，如「这个点周围300米范围的地表温度是多少」',
  ...
  'wf.status.queued': '排队中',
  'wf.progressOf': '执行进度（任务 {n}）',
  'wf.waitAnswer': '等待回答：',
  'wf.cancel': '取消',
  'wf.retry': '重试',
  'wf.pause': '暂停',
  ...
  'task.status.awaiting_info': '等待回答',
  ...
  'node.data_check': '原始包数据检查', 'node.preprocess_split': '预处理与空间划分',
  'node.prep_check': '预处理数据检查', 'node.ttri': 'TTRI 拟合与应用',
  'node.ttri_check': 'TTRI 数据检查', 'node.rf_round': 'RF 模型训练与测试预测',
  'node.train_decision': '训练决定', 'node.promote_best': '登记最佳模型引用',
  'node.tcr': 'TCR 与最终温度', 'node.export': '导出 GeoTIFF',
  'node.closure_eval': '闭合评价', 'node.gapfill': '独立填洞产品',
  'node.rebuild': '重建缺失输入',
  'nodestatus.pending': '等待', 'nodestatus.ready': '就绪',
  'nodestatus.waiting_resource': '等待资源', 'nodestatus.running': '执行中',
  'nodestatus.committing': '提交中', 'nodestatus.succeeded': '完成',
  'nodestatus.failed': '失败', 'nodestatus.waiting_input': '等待回答',
  'nodestatus.retry_wait': '重试等待', 'nodestatus.cancelled': '已取消',
  'nodestatus.blocked': '上游阻塞',
  // 任务结果选择器
  'taskSelect.placeholder': '选择要查看的任务',
  'taskSelect.num': '任务 {n}',
  ...
  'ds.saving': '保存中…',
  'ds.amapKey': '高德 Web 服务 Key（地点与商圈检索，可选）',
  'ds.amapKeyPh': '粘贴高德开放平台「Web 服务」Key',
  'ds.amapKeyHint': '配置后：国内地点/商圈提问由高德检索（中文精度更高）；未配置时自动使用 OpenStreetMap 检索',
  'ds.saveBtn': '保存数据源配置',
  'ds.saved': '已保存并应用',
  ...
  'sa.upload': '点击选择研究区文件',
  'sa.uploaded': '已上传研究区（勾选启用，可多选）',
  'sa.current': '已启用',
  'sa.setCurrent': '设为当前',
  'sa.onlyThis': '仅用这个',
  'sa.activeCount': '已启用 {n} 个',
  'sa.multiNote': '启用多个时，未指名地区的任务会先追问；任务创建后其研究区即固定，不受后续启用变化影响',
  'sa.delTitle': '删除该研究区',
  'sa.delModal': '删除研究区',
  ...
  'fd.taskProducts': '任务产物',
  'fd.taskEmpty': '该任务暂无已登记产物（完成后自动出现）',
  'fd.downloadOne': '下载',
  ...
  'acc.source': '数据来源：{name}',
  'acc.noArtifact': '该任务暂未登记精度产物（执行到精度评估节点后自动出现）',
  ...
  'map.grpAcquire': '数据获取',
  'map.grpResult': '结果',
  'map.noTaskLayers': '该任务暂时没有可展示的影像（执行到数据准备后自动出现）；可在上方选择器切换到其他任务。',
...
const en = {
  ...
  'chat.thinkingEmpty': 'This phase handled understanding, orchestration, location parsing and result statistics ({sec}s total); no expandable model reasoning text was produced.',
  ...
  'chatInput.pointChip': 'Point selected ({lon}, {lat})',
  'chatInput.pointClear': 'Clear',
  'chatInput.pointTitle': 'Map point selected: ask e.g. "LST within 300 m of this point"',
  ...
  'pair.recTag': 'Recommended',
  'pair.lCloud': 'Cloud',
  'pair.lCover': 'Coverage',
  'pair.lCount': ' scenes',
  'pair.lTimeDiff': 'Imaging gap',
  'pair.lDays': ' days',
  ...
  'node.prep_check': 'Preprocessed data check', 'node.ttri': 'TTRI fit & apply',
  'node.ttri_check': 'TTRI data check', 'node.rf_round': 'RF training & test prediction',
  'node.train_decision': 'Training decision', 'node.promote_best': 'Register best model',
  'node.tcr': 'TCR & final temperature', 'node.export': 'Export GeoTIFF',
  'node.closure_eval': 'Closure evaluation', 'node.gapfill': 'Gap-fill product',
  'node.rebuild': 'Rebuild missing inputs',
  'nodestatus.pending': 'Pending', 'nodestatus.ready': 'Ready',
  'nodestatus.waiting_resource': 'Waiting for resources', 'nodestatus.running': 'Running',
  'nodestatus.committing': 'Committing', 'nodestatus.succeeded': 'Done',
  'nodestatus.failed': 'Failed', 'nodestatus.waiting_input': 'Waiting for answer',
  'nodestatus.retry_wait': 'Retry wait', 'nodestatus.cancelled': 'Cancelled',
  'nodestatus.blocked': 'Blocked upstream',
  'taskSelect.placeholder': 'Select a task to view',
  'taskSelect.num': 'Task {n}',
  ...
  'ds.saving': 'Saving…',
  'ds.amapKey': 'Amap Web Service Key (place search, optional)',
  'ds.amapKeyPh': 'Paste an Amap "Web Service" key',
  'ds.amapKeyHint': 'When configured, China place / business-district queries use Amap (better Chinese coverage); otherwise OpenStreetMap is used',
  ...
  'sa.uploaded': 'Uploaded study areas (check to enable, multi-select)',
  'sa.current': 'Enabled',
  'sa.setCurrent': 'Set current',
  'sa.onlyThis': 'Only this',
  'sa.activeCount': '{n} enabled',
  'sa.multiNote': 'With multiple areas enabled, tasks without a named region will ask first; a created task keeps its bound study area.',
  ...
  'fd.taskProducts': 'Task products',
  'fd.taskEmpty': 'No registered artifacts for this task yet (shown after completion)',
  'fd.downloadOne': 'Download',
  ...
  'acc.source': 'Source: {name}',
  'acc.noArtifact': 'No accuracy artifacts registered for this task yet (shown after the accuracy evaluation node runs)',
  ...
  'map.grpAcquire': 'Data Acquisition',
  'map.grpResult': 'Results',
  'map.noTaskLayers': 'No imagery to display for this task yet (shown after data preparation); use the selector above to switch tasks.',
